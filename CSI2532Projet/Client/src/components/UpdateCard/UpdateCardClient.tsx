...
function UpdateCardClient(props: {values: (string | number)[], keys: string[]; tableName: string}) {
  ...
  return (
    <>
      ...
      {modal && <div className="modal-overlay" onClick={()=>setModal(!modal)}></div>}

      {modal && <Modal values={props.values} keys={props.keys} tableName={props.tableName}></Modal>}
    </>
  );
}
